...
func New(pool *pgxpool.Pool) *PlayersRepository {
...
func (r *PlayersRepository) Add(player *domain.Player) error {
	...
	err := r.Pool.QueryRow(ctx, query, player.Club).Scan(&clubId)
	if err != nil {
		fmt.Printf(" error %v\n", err)
		return err
	}
	...
	err = r.Pool.QueryRow(ctx, query, player.Nationality).Scan(&nationalityId)
	if err != nil {
	...
	_, err = r.Pool.Query(ctx, query, player.Name, clubId, nationalityId)

	var id int
	query = `SELECT id FROM Players WHERE name = $1`
	err = r.Pool.QueryRow(ctx, query, player.Name).Scan(&id)
	if err != nil {
		fmt.Printf("list error %v\n", err)
		return err
	}
...
func (r *PlayersRepository) Update(player *domain.Player, id uint) error {
	...
	_, err = r.Pool.Query(ctx, query, player.Name, clubId, nationalityId, id)
	if err != nil {
		fmt.Printf("update error %v\n", err)
		return err
	}
