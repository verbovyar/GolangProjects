...
// 	protoc        (unknown)
// source: google/apiProto/http.apiProto
...
// [HttpRule][google.apiProto.HttpRule], each specifying the mapping of an RPC method
...
// typically specified as an `google.apiProto.http` annotation on the gRPC method.
//
...
//    - Fields referred by the [HttpRule.body][google.apiProto.HttpRule.body]. They are passed via the HTTP
//      request body.
//    - All other fields are passed via the URL query parameters, and the
//      parameter name is the field path in the request message. A repeated
//      field can be represented as multiple query parameters under the same
//      name.
//  2. If [HttpRule.body][google.apiProto.HttpRule.body] is "*", there is no URL query parameter, all fields
//     are passed via URL path and HTTP request body.
//  3. If [HttpRule.body][google.apiProto.HttpRule.body] is omitted, there is no HTTP request body, all
...
// `"{var=*}"`, when such a variable is expanded into a URL path on the transmitter
...
// transmitter side, all characters except `[-_.~/0-9a-zA-Z]` are percent-encoded.
...
// service config is simply the YAML representation of the `google.apiProto.Service`
// apiProto message.
//
// As an alternative to annotating your apiProto file, you can configure gRPC
// transcoding in your service config YAML files. You do this by specifying a
// `HttpRule` that maps the gRPC method to a REST endpoint, achieving the same
// effect as the apiProto annotation. This can be particularly useful if you
// have a apiProto that is reused in multiple services. Note that any transcoding
// specified in the service config will override any matching transcoding
// configuration in the apiProto.
//
// Example:
//
//     http:
...
// apiProto to JSON conversion must follow the [proto3
...
// because transmitter libraries are not capable of handling such variable expansion.
//
...
// no transmitter library can support such complicated mapping.
//
...
type HttpRule struct {
	...
	//
	// Refer to [selector][google.apiProto.DocumentationRule.selector] for syntax details.
...
var file_google_api_http_proto_rawDesc = []byte{
	...
	0x28, 0x09, 0x52, 0x04, 0x70, 0x61, 0x74, 0x68, 0x42, 0x6e, 0x0a, 0x12, 0x63, 0x6f, 0x6d, 0x2e,
	0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65, 0x2e, 0x67, 0x72, 0x70, 0x63, 0x41, 0x70, 0x69, 0x42, 0x09,
	...
	0x6e, 0x73, 0x3b, 0x61, 0x6e, 0x6e, 0x6f, 0x74, 0x61, 0x74, 0x69, 0x6f, 0x6e, 0x73, 0xf8, 0x01,
...
var file_google_api_http_proto_goTypes = []interface{}{
	(*Http)(nil),              // 0: google.apiProto.Http
	(*HttpRule)(nil),          // 1: google.apiProto.HttpRule
	(*CustomHttpPattern)(nil), // 2: google.apiProto.CustomHttpPattern
}
var file_google_api_http_proto_depIdxs = []int32{
	1, // 0: google.apiProto.Http.rules:type_name -> google.apiProto.HttpRule
	2, // 1: google.apiProto.HttpRule.custom:type_name -> google.apiProto.CustomHttpPattern
	1, // 2: google.apiProto.HttpRule.additional_bindings:type_name -> google.apiProto.HttpRule
